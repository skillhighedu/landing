            ...
            >
              {topics.map((topic, idx) => (
                <motion.div
                  ...
                  className="flex gap-3 items-start px-6 py-4 text-sm text-gray-300 hover:bg-neutral-800/40 transition-all rounded-md"
                >
                  <span className="mt-1 w-2 h-2 rounded-full bg-gradient-to-tr from-pink-500 to-violet-400 animate-ping-slow"></span>
                  <p className="leading-relaxed font-bricolage">{topic.contentName}</p>
                </motion.div>
              ))}
            </motion.div>
          )}
