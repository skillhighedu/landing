...
export default function CountdownBanner({
  ...
  const calculateTimeLeft = () => {
    const difference = targetDate ? +new Date(targetDate) - +new Date() : 0;
        ...
        {showEvent && event ? (
          <>
            <div className="flex flex-col gap-2">
              <h3 className="text-lg sm:text-xl tracking-tight">🚀 {event.title}</h3>
              <p className="text-sm font-bricolage text-white/90">
                {event.description}
              </p>
            </div>

            {/* Timer */}
            <div className="flex items-center gap-3 text-xs sm:text-sm font-bricolage tracking-wide
              bg-background/20 dark:bg-background/10
              border border-border/30
              backdrop-blur-md rounded-lg px-4 py-2 shadow-md">
            ...
            <a
              ...
              className="inline-block bg-background/90 text-foreground border border-border
                font-semibold px-5 py-2 rounded-lg shadow-md hover:scale-105 transition-transform animate-pulse"
            >
              {event.buttonText}
            </a>
          </>
            ...
            {timeLeft.days !== "00" ? (
              <>
                <p className="text-sm sm:text-base font-medium tracking-tight">
                  🎉 Use code{" "}
                  <span className="bg-background/20 border border-border/30 px-2 py-0.5 rounded font-bold animate-pulse">
                    LEVELUP
                  </span>{" "}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full mx-auto px-4">
                ...
                <BookingModal
                  title="Book a meet"
                  icon=""
                  className="w-full sm:w-auto bg-background/20 text-white border border-white/20 hover:bg-background/30 font-normal"
                />
              </div>
            )}
          </>
